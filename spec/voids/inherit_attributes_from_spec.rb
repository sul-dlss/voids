...
RSpec.describe 'Voids::Base.inherit_attributes_from' do
  before(:all) do
  ...
  describe 'with only: option' do
    it 'inherits only specified attributes' do
      model_class = @model_class
      form_class = Class.new(Voids::Base) do
        inherit_attributes_from model_class, only: %i[title content]
      end
  ...
  describe 'with except: option' do
    it 'inherits all attributes except specified ones' do
      model_class = @model_class
      form_class = Class.new(Voids::Base) do
        inherit_attributes_from model_class, except: %i[created_at updated_at]
      end
  ...
  describe 'validation' do
    it 'raises error when both only and except are specified' do
      ...
      end.to raise_error(ArgumentError, 'cannot specify both :only and :except')
    end
    ...
    it 'raises error when model class does not respond to attribute_types or columns' do
      ...
      end.to raise_error(ArgumentError, /does not respond to :attribute_types or :columns/)
    end
  end
